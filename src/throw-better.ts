import { world, system, Vector, ItemStack, Player, Entity } from '@minecraft/server';
...
async function fire(player: Player, item, scheduleId = 0) {
	...
	if (playerRotation.x > 30) viewVector = Vector.multiply(viewVector, 2.1);
	const spawnLoc = Vector.add(player.getHeadLocation(), viewVector);
	const projectile = player.dimension.spawnEntity(throwables[item].projectile, spawnLoc) as Entity;
...
async function startReload(player: Player, item: String) {
